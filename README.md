# AI Codex Writing Workbench

一个本地中文写作工作台。

它不是一键生成文章的 SaaS，也不是给所有人直接开箱用的桌面软件。它更像一套可以自己配置的本地写稿系统：Codex / 本地服务负责整理任务和收稿，默认主笔接口负责主稿、选句快改和复核，前端负责把每次修改、喜欢、讨厌和版本保存下来。默认主笔接口使用 OpenAI-compatible `/chat/completions`，可以接你自己的 API。

当前状态：**小范围内测可用**。  
适合能自己运行 Python 本地服务、配置 API Key、接受本地文件存储的用户。
