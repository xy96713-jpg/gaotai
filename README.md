...
稿台是一个可以丢给 Codex 或其他 agent 部署的本地写作台。

你把这个仓库链接发给自己的 agent，让它 clone、复制 `.env.local.example`、填入你的模型 API，然后启动本地服务。打开浏览器后，你得到的不是聊天框，而是一张可以长期用的写作桌。

稿台解决的不是“让 AI 写一篇文章”，而是写完以后怎么改。你可以在正文里选中一句，让模型只改这一句；看到一句好用的表达，就点喜欢；看到一句空话、套话、AI 味重的表达，就点讨厌。这些记录会进入风格库，下次改句、补段、收稿时继续用。

它跑在你自己的电脑上，正文、版本和风格记录都保存在本地。默认模型接口走 OpenAI-compatible `/chat/completions`，所以不绑定某一家 API。
...
- 你已经会用 AI 写文章、口播稿、项目说明，但不想每次都在聊天框里来回复制。
- 你更关心“这篇稿子怎么改好”，不是只要一个完整初稿。
- 你想让系统记住自己的写作口味，比如哪些句子能留，哪些表达以后少出现。
- 你能接受本地 Python 服务、API Key 和本地文件存储。

## 它解决什么

直接让 AI 写中文长稿，经常会出现这些问题：

- 句子很顺，但没有具体对象和动作。
- 标题、段落、总结都有，但整篇找不到几句能直接留下。
- 模型换了一轮，AI 味还在。
- 喜欢和讨厌的写法没有被记住，下次还要重新提醒。
- 写完才发现结构缺层，只能整篇返工。

稿台把写作拆成一条能反复修的流程：

```text
整理材料 -> 写主稿 -> 局部改句 -> 记录偏好 -> 收稿检查 -> 导出 / 演示
```

重点在中间三步：

- **改句**：选中一句，只让模型处理这一句，不把整篇节奏打散。
- **喜欢 / 讨厌**：不是简单点个赞，而是把“这个表达以后多用 / 这种写法以后避开”记下来。
- **风格库**：保存这些判断。下一次生成、补段、收稿时，系统会把你的偏好带进去。
