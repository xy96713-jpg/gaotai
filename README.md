...
稿台是一个本地写作台，给已经在用 AI 写长文的人用。

它不假装“一键生成好文章”。你把稿子放进来，在正文里改，选一句重写，把喜欢和讨厌的表达记下来，最后做一次收稿检查。模型负责给候选、补缺口、查明显问题，稿子仍然在你手里。

它跑在你自己的电脑上。你填一个 OpenAI-compatible `/chat/completions` API，就能启动本地服务和浏览器编辑器。

适合：

- 用 AI 写文章、口播稿、项目说明，但不想整篇丢给聊天框的人。
- 想把“这句我喜欢 / 这句我讨厌”存下来，下次继续用的人。
- 能接受本地 Python 服务、API Key 和本地文件存储的技术用户。

不适合：

- 想要公开 SaaS、账号系统、团队协作或云端同步的人。
- 不想配置 API、不想跑本地服务的人。
...
- 模型换了一轮，AI 味还在。
- 喜欢和讨厌的写法没有被系统记住，下次还要重新提醒。
- 写完才发现结构缺层，只能整篇返工。

这个工作台的目标不是让 AI 一次写完美，而是把写作拆成一条能反复修的流程：
